...
use super::super::{
    ...
    json_i64_or_default, json_string_field, json_string_or_default, json_usize_or_default,
    optional_path_json, optional_string_json,
};

pub(in crate::server) fn handle_root_move(
...
pub(in crate::server) fn handle_root_move_batch_json(
    runtime: &FauplayRuntime,
    request: &str,
) -> HttpResponse {
    let payload =
        match serde_json::from_str::<serde_json::Value>(super::super::http_request_body(request)) {
            Ok(payload) => payload,
            Err(error) => {
                return http_response(400, "Bad Request", &error_json(&error.to_string()));
            }

...
class RetinaFace:
    # Borrowed from insightface.model_zoo.retinaface with minimal changes.
    def __init__(self, model_file: str | None = None, session: onnxruntime.InferenceSession | None = None):
        self.model_file = model_file
        self.session = session
        if self.session is None:
            if self.model_file is None:
                raise RuntimeError("model_file is required when session is None")
    ...
    def _init_vars(self):
        ...
        if len(outputs) == 6:
            ...
            raise RuntimeError(f"Unsupported RetinaFace output count: {len(outputs)}")
    ...
    def prepare(self, ctx_id, **kwargs):
        ...
        if input_size is not None:
            if self.input_size is not None:
                # Keep existing shape when model has fixed input dims.
                pass
            else:
    ...
    def detect(self, image, input_size=None, max_num=0, metric="default"):
        if input_size is None and self.input_size is None:
            raise RuntimeError("input_size is required when model has dynamic input shape")

...
26. 删除撤销成功后，工作区必须恢复删除前快照中的底部结果面板状态，包括面板开闭、显示模式、高度、投射标签集合、活动标签、组级选择态与焦点态。
27. 若删除前活动表面是投射标签，撤销成功后系统必须重新激活该投射标签；若删除前活动表面是目录，则撤销后必须回到目录表面。
...
32. `FR-LFB-32` 删除撤销成功后，系统必须恢复删除前的底部结果面板开闭、显示模式、最近正常高度与投射标签集合，而不是只把文件补回当前列表。
33. `FR-LFB-33` 删除撤销成功后，系统必须恢复删除前的活动表面、目录选择态、投射标签选择态、目录焦点与投射焦点。
...
28. `AC-LFB-28` 在底部结果面板删除文件后执行撤销，面板会恢复到删除前的开闭状态、显示模式、高度、标签集合与活动标签，而不是只恢复文件缩略图。
29. `AC-LFB-29` 在底部结果面板删除文件后执行撤销，组级快捷选择状态、组头计数、勾选集合与焦点项会回到删除前状态。
30. `AC-LFB-30` 若删除前右侧预览或全屏预览正打开被删文件，执行撤销后预览会恢复并定位到该文件的实际恢复路径。
31. `AC-LFB-31` 在 Root A 设置搜索、类型、隐藏空文件夹、排序与标签过滤后刷新页面并重新进入同一 root，这些过滤状态与对应结果集会被恢复。

...
7. 预览纠错面板至少提供：
   - 当前人物信息
   - 推荐人物
   - 统一人物归属输入框（实时补全已有人物，并在输入为全新人物名时提供新建并归入）
   - 移出为未归属
   - 标记忽略
   - 打开人物详情
...
10. 人物视图中的“搜索人物名”只得更新人物列表，不得触发人物工作台全量刷新；输入需做短延迟去抖，避免同一轮键入产生请求风暴或导致 Gateway 超时；搜索语义必须同时匹配真实 `person.name` 与空名人物的占位显示名。
...
16. 双击当前 Root 内的 face 卡片必须切换到来源文件所在目录，并在文件网格与右侧预览中同时定位到同一个来源图片/视频；人物工作台保持打开且不改变当前 face 选择；跨 Root、绝对路径或缺失来源路径的 face 可提示不可跳转，且不得回退到随机文件或目录首项。
17. 人物工作台必须支持将当前已选 faces 的源图片/视频投射到底部结果面板；投射文件列表按源文件去重，保持当前 face 网格中首次出现顺序，投射后自动关闭人物工作台，并让右侧预览与上一项/下一项遍历跟随该投射标签。
18. 预览单脸纠错与人物工作台批量整理中的“归属已有 + 新建人物”必须收敛为同一个人物归属输入框；人物列表搜索、人物重命名与人物合并目标搜索保持独立输入。
19. 统一人物归属输入框必须在键入时以短延迟查询已有人物并显示补全候选；当输入值去空格后与任何已有人物名大小写不敏感精确匹配时，隐藏“新建人物”动作；否则显示“新建并归入 `<输入值>`”动作。
20. 统一人物归属输入框在空输入聚焦时必须展示当前作用域下的默认人物候选，但不得在空输入时显示“新建人物”动作。
21. 顶部工具栏标签过滤必须将人物管理中的 `未归属` 与 `误检/忽略` 暴露为虚拟候选项，分别按“当前文件至少存在一个 `manual_unassigned | deferred | unassigned` face”与“当前文件至少存在一个 `ignored` face”匹配文件结果；它们不是持久化逻辑标签，也不得写回 `vision.face` 或 annotation sidecar。
22. 人物功能链路中的空名人物展示名必须统一回退为 `未命名 <personId前8位>`；人物列表搜索与统一人物归属输入框的精确匹配判定都必须识别该占位显示名。
...
19. `FR-PMFC-19` 人物详情必须支持双击当前 Root face 跳转来源文件预览，并将文件网格精确定位到对应来源文件，且不得改变当前 face 多选状态。
20. `FR-PMFC-20` 人物详情必须支持把已选 faces 的源文件投射为现有 `ResultProjection` 文件列表，并复用底部结果面板的预览遍历语义。
21. `FR-PMFC-21` 预览单脸纠错与人物工作台批量整理必须复用统一的人物归属输入框，实现“补全已有 + 新建并归入”同入口交互。
22. `FR-PMFC-22` 统一人物归属输入框必须支持键盘导航：`ArrowUp/ArrowDown` 切换候选，`Enter` 提交当前高亮候选；若当前无高亮且允许新建，则 `Enter` 执行新建并归入；`Escape` 仅收起候选层。
23. `FR-PMFC-23` 系统必须支持将人物管理 `未归属` 与 `误检/忽略` 作为顶部工具栏标签过滤的虚拟标签候选，并按文件级 face review bucket 命中结果集。
24. `FR-PMFC-24` 系统必须支持用 `未命名 <personId前8位>` 搜索空名人物，并在统一人物归属输入框中把该占位显示名视为现有人物的有效精确匹配。
...
20. `AC-PMFC-20` 双击当前 Root 内 face 后，工作区会切到该来源文件所在目录，文件网格高亮并滚动到同一个来源文件，右侧预览栏打开的也必须是该文件；人物工作台保持打开，双击前的 face 选择集合保持不变；跨 Root 或无来源路径时仅显示不可跳转提示，不得随机回退到其他文件。
21. `AC-PMFC-21` 选中多张 face 后执行“投射源文件”，人物工作台自动关闭，底部结果面板打开并激活“人脸来源”投射标签；同一源文件只出现一次，右侧预览与上一项/下一项遍历仅在该投射列表内移动。
22. `AC-PMFC-22` 在预览单脸纠错中输入已有名字片段时，面板会显示人物补全候选；点击候选或在精确匹配项高亮时按 `Enter`，当前 face 会直接归入该人物。
23. `AC-PMFC-23` 在预览单脸纠错或人物工作台批量整理中输入一个不存在精确重名的新名字时，即使仍有模糊匹配候选，也会显示“新建并归入 `<输入值>`”；直接点击或在无高亮时按 `Enter` 会创建新人物并完成归属。
24. `AC-PMFC-24` 当输入值与某现有人物名大小写不敏感精确相等时，统一人物归属输入框不再显示“新建人物”动作，从而避免通过该入口重复创建同名人物。
25. `AC-PMFC-25` 在当前 Root 顶部工具栏中选择虚拟标签 `人物管理: 未归属` 后，文件结果仅保留至少含一个 `manual_unassigned | deferred | unassigned` face 的图片/视频；选择 `人物管理: 误检/忽略` 后，仅保留至少含一个 `ignored` face 的图片/视频；它们可与普通逻辑标签一起参与 include/exclude 过滤。
26. `AC-PMFC-26` 当某人物 `person.name` 为空时，人物管理列表、人物归属输入框候选与预览纠错面板推荐人物都显示为 `未命名 <personId前8位>`；在人物列表搜索框或统一人物归属输入框中输入该占位显示名时，系统会命中该现有人物，而不是提示新建同名字面人物。

## 10. 默认值与一致性约束 (Defaults & Consistency)

1. v1 不新增普通用户可见的“同资产重新检测并继承旧 faceId”能力。
2. 同一请求内的批量 mutation 默认允许部分成功，不采用全成全败策略。
3. `person.name` 允许为空；人物功能链路中的空名人物展示名固定回退为 `未命名 <personId前8位>`，但该占位名不得写回数据库。

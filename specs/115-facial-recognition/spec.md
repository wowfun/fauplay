...
30. `FR-FACE-30` 预览区对当前图片执行自动检测后，后续自动聚类必须锚定当前 asset，不得仅依赖一个全局 `cluster-pending(limit=...)` 去碰运气命中当前新增 face。
...
29. `AC-FACE-29` 当预览区首次为某张图片自动检测出人脸时，即使全局仍有大量 pending faces，当前图片新增的 face 也会在同一轮补聚类中被处理，而不是长期停留在 `unassigned`。

...
6. WSL 下挂载失效（`No such device`）时支持由 Gateway 统一自动重挂载并单次重试。
...
10. 当目标文件位于 `/mnt/<drive>/...` 且经 Gateway 发起的路径型工具调用报错包含 `No such device` 时，Gateway 需尝试执行 `sudo -S mount -t drvfs <DRIVE>: /mnt/<drive>` 并仅重试一次。
11. 自动重挂载仅使用 Gateway 进程环境变量 `SUDO_PASSWORD`；应用默认从 `~/.fauplay/global/.env` 读取，且不支持 `sudo_password` 兼容读取。
12. Gateway 自动重挂载与探测流程需设置超时并快速失败，避免长时间阻塞导致前端 `MCP_CLIENT_TIMEOUT`。
...
10. `FR-SDS-10` 当本工具经 Gateway 调用且探测阶段命中 `No such device` 时，Gateway 必须按 `/mnt/<drive>` 自动重挂载并单次重试。
11. `FR-SDS-11` 当 `SUDO_PASSWORD` 缺失或挂载失败时，Gateway 必须返回可读错误（含盘符与操作建议）。
12. `FR-SDS-12` 重挂载或探测超时时，Gateway 必须在单次工具调用内快速返回 `MCP_TOOL_CALL_FAILED`，而不是等待客户端超时。
...
10. `AC-SDS-10` `/mnt/e/...` 场景首次探测报 `No such device` 时，Gateway 可自动重挂载后重试成功。
11. `AC-SDS-11` 未在 `~/.fauplay/global/.env` 或其他 Gateway 进程环境中配置 `SUDO_PASSWORD` 时返回明确报错，且不尝试读取 `sudo_password`。
12. `AC-SDS-12` 模拟重挂载命令卡住时，Gateway 在预设超时内返回可读错误，不出现 `Gateway request timed out after 20s`。

...
10. 结果投射标签删除成功后，前端必须立即把已进入全局回收区的文件从当前投射标签移除；若该标签因此为空，必须自动关闭空标签。上述要求同时适用于工作区动作栏与右侧预览动作区触发的删除。
11. 上述删除后的前端收尾必须覆盖按组分行的结果标签：组内缩略图、复选框与组头计数必须同步刷新，不得留下仍可点击的已删残留项。
12. 当结果投射标签删除被撤销时，系统必须通过统一回收站恢复接口把最近删除批次恢复到原路径，并同步恢复删除前的投射标签与预览状态。
...
1. 全局回收区元数据固定存储在 `~/.fauplay/global/` 下；物理托管文件池必须按卷/设备分布在与源文件同卷的受控目录中。
2. 全局回收区必须至少记录：
   - `recycleId`
   - `storedAbsolutePath`
   ...
   - `createdAt`
   - `updatedAt`
3. 当源文件与 `~/.fauplay/global/` 位于同卷时，系统可继续使用 home 下默认托管池；当源文件位于其他卷时，系统必须改用该卷上的受控托管池，而不是跨卷写入 home 托管池。
4. 同卷托管池推荐路径固定为“该卷根路径下的 `.fauplay/global/recycle/files`”；但本规范允许实现以同卷、可恢复、受控为前提选择等价路径。
5. 全局回收区中的文件物理位置不得继续依赖原 Root 的 `.trash` 目录结构。
6. 从结果投射标签进入全局回收区的删除动作必须保留可恢复原路径所需的最小元数据。
...
7. `FR-UTR-07` 全局回收区必须采用“全局元数据 + 同卷托管文件池”模型存储。
8. `FR-UTR-08` 恢复冲突时系统必须自动生成 ` (1)/(2)` 后缀目标名。
9. `FR-UTR-09` 旧 `.trash` 聚合范围在 v1 必须限制为当前 Root，不得隐式扩展到全部历史 Root。
10. `FR-UTR-10` 结果投射标签删除成功后，系统必须同步移除当前投射标签中已进入全局回收区的文件，避免继续把这些文件保留为可删除目标。
11. `FR-UTR-11` 对不与 `~/.fauplay/global/` 同卷的源文件，系统必须改用同卷托管池，避免结果投射标签删除跨设备写入 home 托管池。
12. `FR-UTR-12` 对按组分行的结果标签，删除成功后的前端收尾必须同步刷新组头统计与复选框状态，确保用户无法重新操作已进入全局回收区的文件。
13. `FR-UTR-13` 结果投射标签删除后的撤销必须复用 `POST /v1/recycle/items/restore`，不得新增独立撤销接口。
14. `FR-UTR-14` 结果投射标签删除后的撤销成功时，系统必须以 restore 返回的实际 `nextAbsolutePath` 作为 UI 恢复锚点，避免恢复冲突改名后仍对齐旧路径。
...
9. `AC-UTR-09` 在结果投射标签中删除成功后，当前标签内不会继续显示这些已进入全局回收区的文件；若标签删除后为空，则自动关闭。
10. `AC-UTR-10` 当原文件位于 `/mnt/<drive>/...` 等与 home 不同卷的文件系统时，结果投射标签删除会改用该卷上的受控托管池，不会尝试跨卷写入 `~/.fauplay/global/recycle/files`。
11. `AC-UTR-11` 在按组分行的重复文件结果标签中删除成功后，组头“文件数/已选数”与组内缩略图、复选框会立即同步更新；用户不能继续勾选已进入全局回收区的文件。
12. `AC-UTR-12` 在结果投射标签中删除文件后执行撤销，文件会从统一回收站恢复到原路径或冲突改名后的实际路径，并同步恢复删除前的投射标签内容与预览状态。
...
5. 本专题不新增独立回收站快捷键；删除撤销快捷键归属 `109-soft-delete`。

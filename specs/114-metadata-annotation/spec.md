...
19. 顶部排序条件应支持“标注时间（`updatedAt`）”；应用该排序时，未标注项必须始终置底（不随升降序翻转），未标注组内按名称作为次级排序。
...
21. `FR-MA-21` 文件列表排序条件必须支持 `annotationTime(updatedAt)`；已标注项按 `updatedAt` 升降序排序，未标注项固定置底并保持名称次级排序。
...
23. `AC-MA-23` 选择“标注时间”排序时：已标注项按 `updatedAt` 排序，未标注项始终位于已标注项之后；切换升降序不改变“未标注置底”分组语义。

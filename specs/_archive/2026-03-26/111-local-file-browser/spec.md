...
11. 工作区网格默认展示目录枚举结果；当存在激活中的结果投射时，网格可切换为投射文件列表。
...
4. 信息面板语义
   - 至少展示：`MIME`、`大小`、`修改时间`。
5. 网格数据源语义
   - 目录浏览模式下，网格数据源为当前目录枚举。
   - 结果模式下，网格数据源为某条结果项携带的投射文件列表。
   - 结果模式下不展示目录项。
...
12. `FR-LFB-12` 工作区网格必须允许在目录枚举与结果投射文件列表之间切换数据源。
13. `FR-LFB-13` 结果模式下的文件预览能力判定必须继续复用统一 `FilePreviewKind` 入口。
...
11. `AC-LFB-11` 激活某条结果投射后，工作区网格切换为投射文件列表，且不再显示目录项。
12. `AC-LFB-12` 结果模式下打开图片/视频/文本文件时，仍按统一 `FilePreviewKind` 规则进入对应预览分支。
